...
function loadRows(rows) {
  results = analyzeRows(rows, {today:new Date(), highValue:Number($("highValue").value)||500000});
  state = new Map(results.map(r=>[r.invoice + "|" + Math.random(), "OPEN"]));
  render();
}
...
function render() {
  ...
  $("kpis").innerHTML = [
    ["Outstanding",money(s.outstanding)],["Overdue",money(s.overdue)],
    ["Potential financing",money(s.financing)],["Recovery-ready",money(s.recoveryReady)]
  ].map(([a,b])=>`<div class="kpi"><span>${a}</span><strong>${b}</strong></div>`).join("");
  const queue=results.filter(r=>!r.dataIssue).sort((a,b)=>b.priorityScore-a.priorityScore).slice(0,7);
  ...
  $("issues").textContent=s.dataIssues + " data issues";
  $("count").textContent=s.totalRows + " rows";
}

function card(r,i){
  const key=r.invoice+"|"+i, status=state.get(key)||"OPEN"; state.set(key,status);
  return `<article class="action-card">
    <div class="rank">${i+1}</div><div class="action-main">
      ...
      <div class="buyer">${esc(r.buyer)} · ${money(r.amount)} · ${r.overdue ? r.overdue+"d overdue" : "Not overdue"}</div>
      <p>${esc(r.reason)}</p>
      <div class="why"><b>Do next:</b> ${esc(r.actionSteps[0])}</div>
      <div class="card-actions"><button data-key="${key}" data-state="IN PROGRESS" class="smallbtn">Start</button><button data-key="${key}" data-state="DONE" class="smallbtn secondary">Done</button></div>
    </div></article>`;
}

function table(rows){
  return `<div class="table-wrap"><table><thead><tr><th>Priority</th><th>Invoice</th><th>Buyer</th><th>Amount</th><th>Age</th><th>Action</th><th>Evidence</th><th>Status</th></tr></thead><tbody>${rows.map((r,i)=>`<tr><td><span class="priority ${r.priority.toLowerCase()}">${r.priority}</span></td><td><b>${esc(r.invoice)}</b></td><td>${esc(r.buyer)||"—"}</td><td>${money(r.amount)}</td><td>${r.dataIssue?"—":r.overdue+"d"}</td><td><span class="badge ${badgeClass(r.action)}">${esc(r.action)}</span></td><td>${r.dataIssue?"—":r.evidenceCompleteness+"%"}</td><td>${r.dataIssue?"DATA ISSUE":(state.get(r.invoice+"|"+i)||"OPEN")}</td></tr>`).join("")}</tbody></table></div>`;
}

$("csv").value = demo.map(r=>r.join(",")).join("\n");
$("analyze").onclick=()=>loadRows(parseCSV($("csv").value));
$("sample").onclick=()=>{ $("csv").value=demo.map(r=>r.join(",")).join("\n"); loadRows(demo.map(r=>Object.fromEntries(headers.map((h,i)=>[h,r[i]]))); };
$("clear").onclick=()=>{ $("csv").value=""; results=[]; render(); };
$("queue").addEventListener("click",e=>{const b=e.target.closest("button");if(!b)return;const idx=[...$("queue").querySelectorAll("button")].indexOf(b);const cards=$("queue").querySelectorAll(".action-card");const key=cards[Math.floor(idx/2)].querySelector("button").dataset.key;state.set(key,b.dataset.state);render()});
